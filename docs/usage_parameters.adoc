...
|`-searchWindow`
|An integer
|Determine the extent to which current recognized word is searched in the respective subtitle dialogue. Default value is 3.
...
|`-- display-recognized`
|`yes`, `no`
|Determine whether to display the recognized words and matching status on stdout or not.
